...
def import_activity_file(
    ...
    try:
        ...
        try:
            ...
            if existing:
                return ImportResult(
                    status="duplicate",
                    file_name=file_name,
                    message=f"{tr('Archivo ya importado')}: {existing.original_name}",
                )
            ...
            if parsed.started_at and parsed.duration_sec:
                ...
                if dup:
                    ...
                    return ImportResult(
                        status="duplicate",
                        file_name=file_name,
                        message=f"{tr('Actividad ya existente (coincide fecha y duración)')}: {dup.file_name}",
                    )
            ...
            return ImportResult(
                status="created",
                file_name=file_name,
                message=tr("Importado correctamente"),
                activity_id=activity.id,
            )
        ...
        traceback.print_exc()
        return ImportResult(status="error", file_name=file_name, message=f"{tr('Error inesperado')}: {e}")

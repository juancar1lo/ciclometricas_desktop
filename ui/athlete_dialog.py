...
class NewAthleteDialog(QDialog):
    ...
    def __init__(self, manager: AthleteManager, parent: QWidget | None = None):
        ...
        self.setWindowTitle(tr("Nuevo perfil de atleta"))
        ...
        # Título
        title = QLabel(f"🚴  {tr('Crear perfil')}")
        title.setStyleSheet(
        ...
        desc = QLabel(tr("Introduce los datos básicos del ciclista. Puedes añadir una foto de perfil."))
        ...
        photo_hint = QLabel(tr("JPG o PNG, máx 2 MB"))
        ...
        self.name_input.setPlaceholderText(tr("Ej: Carlos, María..."))
        self.name_input.setMaxLength(50)
        form.addRow(self._form_label(tr("🆔 Nombre:")), self.name_input)

        self.ftp_input = QSpinBox()
        self.ftp_input.setRange(50, 600)
        self.ftp_input.setValue(200)
        self.ftp_input.setSuffix(" W")
        form.addRow(self._form_label(f"⚡ {tr('FTP')}:"), self.ftp_input)
        ...
        form.addRow(self._form_label(tr("🏋️ Peso:")), self.weight_input)
        ...
        form.addRow(self._form_label(tr("❤️ FC máx:")), self.hr_max_input)
        ...
        self.hr_lthr_input.setToolTip(tr("Frecuencia cardíaca de umbral anaeróbico (LTHR)"))
        form.addRow(self._form_label(f"🫀 {tr('FCL')}:"), self.hr_lthr_input)
    ...
    def _on_select_photo(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self,
            tr("Seleccionar foto de perfil"),
            str(Path.home()),
            tr("Imágenes (*.png *.jpg *.jpeg *.bmp *.webp);;Todos (*)"),
        )
        if not path:
            return
        p = Path(path)
        if p.stat().st_size > 2 * 1024 * 1024:  # 2 MB
            QMessageBox.warning(self, tr("Archivo demasiado grande"),
                                tr("La foto no puede superar 2 MB. Elige otra."))
            return
    ...
    def _on_accept(self) -> None:
        ...
        if not name:
            QMessageBox.warning(self, tr("Error"), tr("El nombre no puede estar vacío."))
            return
        try:
...
class AthleteChooserDialog(QDialog):
    ...
    def __init__(self, manager: AthleteManager, parent: QWidget | None = None):
        super().__init__(parent)
        self.manager = manager
        self.setWindowTitle(tr("Ciclométricas — Elegir atleta"))
        ...
        subtitle = QLabel(tr("Selecciona un perfil de atleta para continuar."))
    ...
    def _on_delete(self) -> None:
        ...
        if confirmar(
            ...
            f"{tr('¿Eliminar el perfil')} «{name}» {tr('y todos sus datos?')}\n\n{tr('Esta acción no se puede deshacer.')}",
        ):

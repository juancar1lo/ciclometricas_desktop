...
<style>
  html, body { margin:0; padding:0; width:100%%; height:100%%;
               background:__BG_COLOR__; overflow:hidden; }
...
(function(){
  ...
  function init() {
    ...
    var map = L.map('map', {scrollWheelZoom: false, zoomControl: true, dragging: true});
...
def _build_map_html(
    ...
    return html.replace("__BG_COLOR__", "#111827")
...
class RouteMapWidget(QFrame):
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel(tr("📍 Mapa de la ruta"))
        title.setStyleSheet(
        ...
        for key, label, available in [
            ("power", tr("⚡ Potencia"), self._has_power),
            ("hr",    tr("❤️ FC"),       self._has_hr),
            ("speed", tr("💨 Velocidad"), self._has_speed),
        ]:
            if not available:
                continue
    ...
    def _build_legend(self) -> QFrame:
        ...
        for text in (tr("Bajo"), "Medio", tr("Alto")):
            lbl = QLabel(text)
            lbl.setStyleSheet(
                f"font-size: {FONT_SIZE_XS}; color: {COLORS['fg_dim']}; background: transparent;"
            )
            alignment = (
                Qt.AlignmentFlag.AlignLeft  if text == tr("Bajo")
                else Qt.AlignmentFlag.AlignRight if text == tr("Alto")
    ...
    def _mode_desc(mode: str) -> str:
        return {
            "power": tr("Ruta coloreada por potencia (W)."),
            "hr":    tr("Ruta coloreada por frecuencia cardíaca (bpm)."),
            "speed": tr("Ruta coloreada por velocidad (km/h)."),
        }.get(mode, "")

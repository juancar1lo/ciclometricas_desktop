...
from calc.torque import compute_torque_series
...
class TimeSeriesChart(QFrame):
    ...
    def __init__(
        ...
        # Torque (AET) — requiere potencia + cadencia
        self._has_torque = self._has_power and self._has_cadence
        self._torque_pts = []
        if self._has_torque:
            self._torque_pts = compute_torque_series(self._samples)
            if len(self._torque_pts) < 5:
                self._has_torque = False
                self._torque_pts = []

        # Mapa rápido t → torque para tooltip
        self._torque_map: Dict[int, float] = {}
        if self._torque_pts:
            self._torque_map = {int(p.t): p.torque for p in self._torque_pts}
    ...
    def _build_ui(self):
        ...
        title = QLabel(tr("📈 Series temporales"))
        title.setStyleSheet(
            f"font-size: {FONT_SIZE_XL}; font-weight: bold; "
            f"color: {COLORS['fg']}; background: transparent;"
        )
        title_col.addWidget(title)
        desc = QLabel(tr("Potencia, FC, cadencia, velocidad y altitud a lo largo del entrenamiento."))
        desc.setStyleSheet(
        ...
        toggle_defs = [
            ("altitude", tr("Altitud"),   self._has_alt,     SERIES_COLORS["altitude"]),
            ("wbal",     tr("W' bal"),    self._has_wbal,    SERIES_COLORS["wbal"]),
            ("hr",       tr("FC"),        self._has_hr,      SERIES_COLORS["hr"]),
            ("cadence",  tr("Cadencia"),  self._has_cadence, SERIES_COLORS["cadence"]),
            ("speed",    tr("Velocidad"), self._has_speed,   SERIES_COLORS["speed"]),
            ("torque",   "Par (Nm)",  self._has_torque,  SERIES_COLORS["torque"]),
        ]
    ...
    def _draw(self):
        ...
        # Torque: escalado a ~40% de y para no dominar el gráfico
        if self._show.get("torque") and self._has_torque:
            torque_vals = [p.torque for p in self._torque_pts]
            if torque_vals:
                self._torque_max_raw = float(max(torque_vals))
        ...
        if self._show.get("cadence") and self._has_cadence:
            ...
            self._pw.plot(
                ...
                name=tr("Cadencia (rpm)"),
            )

        # ── TORQUE (escalado a y_top * 0.40)
        if self._show.get("torque") and self._has_torque and self._torque_pts:
            torque_max = getattr(self, "_torque_max_raw", 60)
            if torque_max <= 0:
                torque_max = 60
            target_top = y_top * 0.40
            t_ts = np.array([p.t for p in self._torque_pts], dtype=float)
            t_vals = np.array([
                p.torque / torque_max * target_top for p in self._torque_pts
            ], dtype=float)
            self._pw.plot(
                t_ts, t_vals,
                pen=pg.mkPen(color=SERIES_COLORS["torque"], width=1.5),
                name=tr("Par (Nm)"),
            )
    ...
    def _attach_tooltip(self, ts: np.ndarray):
        ...
        def format_fn(x: float, y: float) -> str:
            ...
            if has["power"] and show.get("power") and s.get("p") is not None:
                lines.append(tooltip_line(tr("Potencia"), f"{int(s['p'])} W", SERIES_COLORS["power"]))

            if has["wbal"] and show.get("wbal"):
                t_int = int(s.get("t", 0))
                pct = wbal_map.get(t_int)
                if pct is not None:
                    lines.append(tooltip_line(tr("W' bal"), f"{pct}%", SERIES_COLORS["wbal"]))

            if has["hr"] and show.get("hr") and s.get("hr") is not None:
                lines.append(tooltip_line(tr("FC"), f"{int(s['hr'])} bpm", SERIES_COLORS["hr"]))

            if has["cadence"] and show.get("cadence") and s.get("c") is not None:
                lines.append(tooltip_line(tr("Cadencia"), f"{int(s['c'])} rpm", SERIES_COLORS["cadence"]))

            if has["speed"] and show.get("speed"):
                spd = s.get("v") or s.get("s")
                if spd is not None:
                    spd_kmh = spd * 3.6  # m/s → km/h
                    lines.append(tooltip_line(tr("Velocidad"), f"{spd_kmh:.1f} km/h", SERIES_COLORS["speed"]))

            if has["altitude"] and show.get("altitude") and s.get("alt") is not None:
                lines.append(tooltip_line(tr("Altitud"), f"{int(s['alt'])} m", SERIES_COLORS["altitude"]))

            if has["torque"] and show.get("torque"):
                t_int = int(s.get("t", 0))
                trq = torque_map.get(t_int)
                if trq is not None:
                    lines.append(tooltip_line(tr("Par"), f"{trq:.1f} Nm", SERIES_COLORS["torque"]))

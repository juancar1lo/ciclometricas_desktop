...
# Colores fijos de series (no dependen del tema)
_SERIES_COLORS = {
    "ctl":      "#FF9149",
    "atl":      "#FF6363",
    "tsb":      "#60B5FF",
...
def make_plot(
    ...
    """Crea un PlotWidget con estilo adaptado al tema."""
    if bg is None:
        bg = CHART_COLORS["bg"]
    pw = pg.PlotWidget()
...
_MONTH_SHORT = {
    1: tr("ene"), 2: tr("feb"), 3: tr("mar"), 4: tr("abr"),
    5: tr("may"), 6: tr("jun"), 7: tr("jul"), 8: tr("ago"),
    9: tr("sept"), 10: tr("oct"), 11: tr("nov"), 12: tr("dic"),
}
...
_TOOLTIP_FG = "#f1f5f9"
_TOOLTIP_MUTED = "#94a3b8"


def tooltip_line(label: str, value: str, color: str | None = None) -> str:
    """Genera una línea HTML coloreada para un tooltip.

    Ejemplo: tooltip_line(tr("CTL"), "72.3", "#FF9149") +
    """
    if color is None:
        color = _TOOLTIP_FG
    return (
        f'<span style="color:{color};font-weight:600">{label}:</span> '
        f'<span style="color:{_TOOLTIP_FG}">{value}</span>'
    )


def tooltip_header(text: str) -> str:
    """Genera una cabecera HTML para un tooltip."""
    return f'<span style="color:{_TOOLTIP_MUTED};font-weight:700">{text}</span>'
...
class ChartTooltip:
    ...
    def __init__(
        ...
        # Etiqueta flotante — fondo opaco oscuro
        _lbl_fg, _lbl_bg, _lbl_border = "#f1f5f9", "#0f172a", "#334155"
        self.label = pg.TextItem(
            "",
            color=_qcolor(_lbl_fg),
            anchor=(0, 1),
            fill=_qcolor(_lbl_bg, 240),
            border=pg.mkPen(_lbl_border, width=1),
        )

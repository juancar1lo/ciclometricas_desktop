...
# Las vistas se importan bajo demanda en _ensure_view() para acelerar el arranque
...
_SECTION_DEFS = [
    ("dashboard",          "Panel",             "📊"),
    ("summary",            "Resumen",           "📈"),
    ("monotony",           "Monotonía",         "❤️"),
    ("race_readiness",     "Preparación",       "🎯"),
    ("fatigue_resistance", "Res. Fatiga",       "📉"),
    ("durability",         "Durabilidad",       "🧪"),
    ("recovery",           "Recuperación",      "🔋"),
    ("health",             "Salud",             "❤️‍🩹"),
    ("activities",         "Entrenamientos",    "📋"),
    ("spike_editor",      "Editor de picos",   "✂️"),
    ("import",             "Importar archivos", "📥"),
    ("ai_coach",           "Consejero IA",      "🤖"),
    ("settings",           "Configuración",     "⚙️"),
]


def _get_sections():
    """Devuelve SECTIONS con labels traducidas (evaluadas en runtime)."""
    return [{"key": k, "label": tr(lbl), "icon": ico} for k, lbl, ico in _SECTION_DEFS]
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, manager: AthleteManager, profile: AthleteProfile):
        ...
        # Fondo oscuro inmediato para evitar flash blanco al iniciar
        ...
        # Registro de vistas ya inicializadas (lazy loading)
        self._initialized_views: set[str] = set()

        # Solo cargamos el dashboard al inicio para arrancar rápido
    ...
    def _build_sidebar(self) -> QFrame:
        ...
        btn_switch = QPushButton(tr("Cambiar atleta"))
    ...
    def _init_dashboard(self) -> None:
        """Carga solo el dashboard al arrancar (rápido)."""
        ...
        self._initialized_views.add("dashboard")

    def _ensure_view(self, key: str) -> None:
        """Importa y construye una vista bajo demanda (lazy loading)."""
        if key in self._initialized_views:
            return
        ...
        if key == "import":
            ...
            _oll_url = self.profile.config.get("ollama_url", "http://localhost:11434")
            _oll_model = self.profile.config.get("ollama_model", "deepseek-r1:8b")
            from services.ai_coach import get_ai_coach
            get_ai_coach(ollama_url=_oll_url, model=_oll_model)
            from ui.views.ai_coach_view import AiCoachView
            self._ai_coach_view = AiCoachView()
            self.set_view("ai_coach", self._ai_coach_view)

        elif key == "spike_editor":
            from ui.views.spike_editor_view import SpikeEditorView
            self._spike_editor_view = SpikeEditorView()
            ftp = self.profile.config.get("ftp", 200)
            weight = self.profile.config.get("weight_kg", 70)
            self._spike_editor_view.load_from_profile(ftp, weight)
            self.set_view("spike_editor", self._spike_editor_view)

    def _on_import_completed(self) -> None:
        if hasattr(self, "_activities_view"):
    ...
    def _create_placeholder(self, section_name: str, icon: str = "🚧") -> QWidget:
        ...
        sub = QLabel(tr("Esta sección se implementará en las próximas fases."))
    ...
    def _navigate(self, key: str) -> None:
        ...
        # Lazy loading: importar y construir la vista si aún no existe
    ...
    def _open_activity_detail(self, activity_id: int) -> None:
        ...
        print(f"[MainWindow] _open_activity_detail called with id={activity_id}")
        try:
            ...
            print(f"[MainWindow] Detail view set successfully for id={activity_id}")
        except Exception as e:
            print(f"[MainWindow] ERROR opening detail: {e}")

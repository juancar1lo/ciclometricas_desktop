...
class MonotonyView(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        lay.addWidget(_section_title(
            "❤️", tr("Monotonía"),
            tr("Índice de monotonía de Foster · Variabilidad del entrenamiento")
        ))
        ...
        self._months_combo.addItems([
            tr("Últimos 3 meses"), tr("Últimos 6 meses"),
            tr("Último año"), tr("Últimos 2 años"),
        ])
        ...
        for color, label in [(COLOR_LOAD, tr("Carga semanal (TSS)")), (COLOR_MONOTONY, tr("Monotonía"))]:
            dot = QLabel("■")
        ...
        # Table
        lay.addWidget(_section_title("📋", tr("Detalle semanal"), tr("TSS diario y monotonía por semana")))
    ...
    def _make_table(self) -> QTableWidget:
        cols = [tr("Semana"), tr("Carga")] + _DAY_LABELS + [tr("Días"), "Mon.", tr("Estado")]
        tbl = QTableWidget()
    ...
    def _make_explanation(self) -> QFrame:
        ...
        title = QLabel(tr("📖  Cómo leer esta métrica"))
        title.setStyleSheet(f"font-size: 14px; font-weight: 700; color: {COLORS['fg']}; border: none;")
        lay.addWidget(title)

        texts = [
            (tr("El modelo de <b>Foster (1998)</b> mide la <b>variabilidad</b> de tu ") +
             tr("entrenamiento semanal, algo que el modelo PMC (CTL/ATL/TSB) no captura.")),
            (f"<span style='color:{COLOR_MONOTONY};font-weight:600'>{tr('Monotonía = Media / Desv. Estándar')}</span><br/>" +
             tr("Mide cuán uniforme es tu entrenamiento. Valores altos (&gt; 2.0) indican poca variabilidad.")),
            (tr("<span style='color:#22c55e'>Monotonía &lt; 1.5 — Buena variabilidad.</span> ") +
             tr("Entrenamientos variados: días duros + días suaves + descanso. Ideal.")),
            (tr("<span style='color:#ef4444'>Monotonía &gt; 2.0 — ⚠️ Riesgo.</span> ") +
             tr("Todos los días con TSS similar. Añade días de descanso o varía la intensidad.")),
            (tr("<b>Consejo:</b> Alterna días de alta carga (intervalos, series) con días ") +
             tr("de recuperación activa (Z1-Z2) y al menos un día de descanso completo por semana.")),
        ]
        for t in texts:
            lbl = QLabel(t)
    ...
    def _build_data(self, months: int):
        ...
        while cur <= last_mon:
            ...
            _ms = _month_short_map()
            week_label = f"{cur.day} {_ms[cur.month]} – {sun.day} {_ms[sun.month]}"
    ...
    def _update_cards(self) -> None:
        ...
        self._cards_row.addWidget(_stat_card(
            tr("Semanas analizadas"),
            str(len(valid)),
            COLOR_LOAD,
            tr("con datos suficientes"),
        ))
        self._cards_row.addWidget(_stat_card(
            tr("Objetivo"),
            "< 1.5",
    ...
    def _draw_chart(self) -> None:
        ...
        if not self._weeks:
            txt = pg.TextItem(tr("Sin datos para el periodo seleccionado"), color=QColor(COLORS["fg_muted"]))
        ...
        p2.setXLink(p1)
        p1.getAxis("right").setLabel(tr("Monotonía"), color=COLOR_MONOTONY)
        ...
        def _fmt(xv, yv):
            ...
            lines = [
                ...
                tooltip_line(tr("Carga semanal"), f"{w['week_load']:.0f} TSS", COLOR_LOAD),
                tooltip_line(tr("Media diaria"), f"{w['mean_daily_tss']:.0f} TSS · DE: {w['std_daily_tss']:.1f}"),
                tooltip_line(tr("Días activos"), f"{w['active_days']}/7"),
            ]
            if w["monotony"] is not None:
                lines.append(tooltip_line(tr("Monotonía"), f"{w['monotony']} — {w['class_label']}", COLOR_MONOTONY))
            # Mini daily
            day_str = "  ".join(

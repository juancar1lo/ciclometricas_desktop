...
from services.ai_coach import get_ai_coach, DEFAULT_OLLAMA_URL, DEFAULT_MODEL
...
class SettingsView(QWidget):
    ...
    def __init__(self, manager: AthleteManager, profile: AthleteProfile, parent=None):
        ...
        # Título
        title = QLabel(tr("\u2699\ufe0f  Configuración"))
        title.setStyleSheet(
            f"font-size: {FONT_SIZE_TITLE}; font-weight: bold; color: {COLORS['fg']};"
        )
        layout.addWidget(title)
        desc = QLabel(tr("Define tu FTP, peso, FC máx, FCL (umbral) y registra tests de potencia para calcular CP, W\u2032 y VO2max."))
        ...
        self.tabs.addTab(self._build_profile_tab(), tr("Perfil"))
        self.tabs.addTab(self._build_zones_tab(), tr("Zonas Coggan"))
        self.tabs.addTab(self._build_history_tab(), tr("Historial"))
        self.tabs.addTab(self._build_tests_tab(), tr("Tests de potencia"))
        self.tabs.addTab(self._build_strava_tab(), tr("Strava"))
        self.tabs.addTab(self._build_backup_tab(), tr("Copia de seguridad"))
        self.tabs.addTab(self._build_ia_tab(), tr("IA"))
    ...
    def _build_profile_tab(self) -> QWidget:
        ...
        card = _make_card(f"🧑‍🚀  {tr('Tus datos actuales')}",
                          tr("Cada cambio se guarda con fecha. Así puedes ver cómo evoluciona tu FTP y tu peso."))
        form = QFormLayout()
        form.setSpacing(14)
        ...
        form.addRow(tr("\U0001F3CB\ufe0f Peso (kg):"), self.weight_input)

        self.hr_max_input = QSpinBox()
        self.hr_max_input.setRange(100, 230)
        self.hr_max_input.setValue(config.get("hr_max", 185))
        self.hr_max_input.setSuffix(f" {hr_unit()}")
        form.addRow(f"❤️ {tr('FC máx')} ({hr_unit()}):", self.hr_max_input)

        self.hr_lthr_input = QSpinBox()
        self.hr_lthr_input.setRange(0, 220)
        self.hr_lthr_input.setValue(config.get("hr_lthr", 0))
        self.hr_lthr_input.setSuffix(f" {hr_unit()}")
        self.hr_lthr_input.setSpecialValueText(tr("Opcional — sin dato"))
        form.addRow(f"💓 {tr('FCL / LTHR')} ({hr_unit()}):", self.hr_lthr_input)
        ...
        form.addRow(tr("Notas (opcional):"), self.notes_input)
        ...
        # Botón guardar
        btn_row = QHBoxLayout()
        self.btn_save = QPushButton(tr("\U0001F4BE  Guardar cambios"))
        ...
        lang_lbl.setStyleSheet(f"font-size: {FONT_SIZE_BASE}; color: {COLORS['fg_muted']};")
        lang_row.addWidget(lang_lbl)
        self._lang_combo = QComboBox()
        self._lang_combo.addItem("\U0001f1ea\U0001f1f8  Espa\u00f1ol", "es")
    ...
    def _build_zones_tab(self) -> QWidget:
        ...
        card = _make_card(tr("\u03a3  Cálculo de zonas Coggan"),
                          tr("Elige a partir de qué umbral se calculan las 7 zonas de potencia. ") + tr("Las zonas son siempre porcentajes de la referencia (FTP, CP o mFTP)."))
        ...
        options = [
            ("ftp", tr("FTP"), tr("Tu Functional Threshold Power configurado"), ftp_val),
            ("cp", tr("CP"), tr("Critical Power del último test"), cp_val),
            ("mftp", "mFTP", tr("FTP modelado (0.96 · CP)"), mftp_val),
        ]
        ...
        note = QLabel(
            tr("Las zonas se recalculan automáticamente en el panel usando la referencia elegida. ") +
            tr("El bucketing de actividades antiguas conserva el FTP que tenías cuando las subiste.")
        )
    ...
    def _build_history_tab(self) -> QWidget:
        ...
        card = _make_card(f"🕙  {tr('Historial de tu perfil')}",
                          tr("Cada vez que actualizas tu FTP, peso o FC máx se guarda una entrada. ") +
                          tr("Puedes eliminar las que ya no sean relevantes."))
        ...
        cols = [tr("Fecha"), tr("FTP"), tr("Peso"), tr("FC máx"), "FCL", tr("Notas"), "", ""]
    ...
    def _build_tests_tab(self) -> QWidget:
        ...
        card_new = _make_card(tr("\U0001F9EA  Nuevo test de potencia"),
                              tr("Introduce tus 3 tests: corto (30s–2min), medio (3–5min) y largo (10–20min). ") +
                              tr("Se calculará CP, W\u2032, mFTP y VO2max."))
        ...
        lbl_sprint = QLabel(tr("Sprint máx (opc.):"))
        ...
        self.max_power_input.setSpecialValueText(tr("Opcional — sin dato"))
        ...
        self.btn_add_test = QPushButton(tr("  ➕  Calcular y guardar test  "))
        ...
        # ── Card "Último modelo" ──
        self.last_model_card = _make_card(tr("📊  Último modelo"),
                                           tr("Resultados del modelo CP más reciente."))
        ...
        card_list = _make_card(f"🕘  {tr('Historial de tests')}",
                               tr("Elimina los tests pasados que ya no quieras conservar para refinar el modelo."))
        ...
        test_cols = [
            tr("Fecha"), tr("Corto"), tr("Medio"), tr("Largo"), tr("P máx"),
            tr("CP"), tr("W'"), "P5min", "mFTP", tr("VO2max"), tr("Fiabilidad"), "",
        ]
    ...
    def _on_save_profile(self) -> None:
        ...
        QMessageBox.information(self, tr("Guardado"), tr("Perfil actualizado correctamente."))
    ...
    def _on_add_test_safe(self) -> None:
        ...
        try:
            ...
            QMessageBox.critical(
                self, tr("Error inesperado"),
                f"{tr('Ocurrió un error al calcular/guardar el test')}:\n\n{exc}\n\n" +
                tr("Revisa la consola (CMD) para más detalles."),
            )
    ...
    def _on_add_test(self) -> None:
        ...
        if missing:
            QMessageBox.warning(
                self, tr("Datos incompletos"),
                f"{tr('Falta potencia en')}: {', '.join(missing)}.\n\n" +
        ...
        if not model:
            QMessageBox.warning(self, tr("Error"), tr("No se pudo calcular el modelo CP. Verifica los datos."))
            return
        ...
        self._refresh_zones_values()
        self.profile_updated.emit()
        QMessageBox.information(self, tr("Test guardado"),
    ...
    def _on_delete_snapshot(self, snap_id: int) -> None:
        if not confirmar(self, tr("Confirmar"), tr("¿Eliminar esta entrada del historial?")):
            return
        session = get_session()
        try:
            snap = session.query(ProfileSnapshot).filter_by(id=snap_id).first()
    ...
    def _on_delete_test(self, test_id: int) -> None:
        if not confirmar(self, tr("Confirmar"), tr("¿Eliminar este test de potencia?")):
            return
        session = get_session()
        try:
    ...
    def _refresh_history(self) -> None:
        ...
        for row, s in enumerate(snaps):
            ...
            self.history_table.setItem(row, 3, QTableWidgetItem(f"{s.hr_max} {hr_unit()}"))
            self.history_table.setItem(row, 4, QTableWidgetItem(
                f"{s.hr_lthr} {hr_unit()}" if s.hr_lthr else "—"))
            ...
            btn_edit.setToolTip(tr("Editar nota"))
            ...
            if not is_only:
                ...
                btn.setToolTip(tr("Eliminar entrada"))
    ...
    def _refresh_tests(self) -> None:
        ...
        for row, t in enumerate(tests):
            ...
            btn.setFixedSize(34, 34)
            btn.setProperty("class", "ghost")
            btn.setToolTip(tr("Eliminar test"))
    ...
    def _populate_last_model(self, t: PowerTestSet | None) -> None:
        """Rellena la card tr('Último modelo') con los datos del test más reciente."""
        ...
        if t is None:
            lbl = QLabel(tr("Aún no hay tests registrados."))
        ...
        rows = [
            (tr("CP"), f"{t.cp:.0f} W" if t.cp else "—"),
            (tr("W'"), f"{t.w_prime / 1000:.1f} kJ" if t.w_prime else "—"),
            ...
            (tr("P MÁX"), f"{t.max_power} W" if t.max_power else "—"),
        ]
        ...
        if t.r_squared is not None:
            ...
            fiab_text = "—"
        fiab_row = _row(tr("Fiabilidad"), fiab_text)
    ...
    def _build_strava_tab(self) -> QWidget:
        ...
        card = _make_card(f"🔗  {tr('Strava')}",
                          tr("Vincula tu cuenta de Strava para importar actividades de ciclismo."))
    ...
    def _refresh_strava_status(self) -> None:
        ...
        if is_connected:
            athlete_name = strava_data.get("athlete_name", tr("Atleta de Strava"))
            ...
            btn_row = QHBoxLayout()
            btn_import = QPushButton(f"📥  {tr('Importar actividades')}")
            btn_import.setFixedHeight(38)
            btn_import.clicked.connect(self._on_strava_import)
            btn_row.addWidget(btn_import)
            btn_disconnect = QPushButton(f"🔌  {tr('Desvincular')}")
            ...
            note = QLabel(
                tr("Las actividades importadas desde Strava aparecerán en tu panel ") +
                tr("con todos los datos de potencia, FC y cadencia disponibles.\n") + tr("Se detectan duplicados automáticamente (por fecha y duración).")
            )
            ...
            desc = QLabel(
                tr("Al vincular tu cuenta de Strava, podrás importar tus actividades de ciclismo ") +
                tr("con todos los datos de potencia, frecuencia cardíaca y cadencia.\n") + tr("La subida manual de archivos .fit/.tcx seguirá disponible.")
            )
            ...
            btn_connect = QPushButton(f"🔗  {tr('Vincular con Strava')}")
            ...
            info = QLabel(
                tr("Se abrirá tu navegador web para autorizar el acceso.\n") +
                tr("Necesitarás un Client ID y Client Secret de la API de Strava. ") +
                tr("Puedes obtenerlos en") + " https://www.strava.com/settings/api"
            )
    ...
    def _on_strava_connect(self) -> None:
        ...
        dlg = QDialog(self)
        dlg.setWindowTitle(tr("Vincular con Strava"))
        ...
        inp_id = QLineEdit()
        inp_id.setPlaceholderText(tr("Client ID"))
        dlg_lay.addWidget(inp_id)
        inp_secret = QLineEdit()
        inp_secret.setPlaceholderText(tr("Client Secret"))
        ...
        if not client_id or not client_secret:
            QMessageBox.warning(self, tr("Faltan datos"), tr("Introduce Client ID y Client Secret."))
            return
        ...
        self._oauth_progress = QProgressDialog(
            tr("Esperando autorización de Strava...\n") +
            tr("Autoriza en el navegador y espera."),
            tr("Cancelar"), 0, 0, self
        )
        self._oauth_progress.setWindowTitle(tr("Vincular con Strava"))
        ...
        self._oauth_state = {
            "code": None, tr("error"): None, "done": False,
        ...
        def _bg_wait_for_code():
            ...
            try:
                ...
                self._oauth_state[tr("error")] = err
            except Exception as e:
                self._oauth_state[tr("error")] = str(e)
            finally:
    ...
    def _poll_oauth_result(self) -> None:
        ...
        if state[tr("error")]:
            QMessageBox.critical(self, tr("Error"), state[tr("error")])
            return
        ...
        self.manager.save_config(self.profile)
        self._refresh_strava_status()
        QMessageBox.information(self, tr("Strava vinculado"),
            f"{tr('¡Cuenta vinculada correctamente!')}\n{tr('Atleta')}: {tokens.athlete_name}")

    def _on_strava_disconnect(self) -> None:
        if not confirmar(self, "Desvincular Strava",
                         tr("¿Desvincular tu cuenta de Strava?\n") +
                         tr("Las actividades ya importadas se mantendrán.")):
            return
        self.profile.config["strava"] = None
        self.manager.save_config(self.profile)
        self._refresh_strava_status()
        QMessageBox.information(self, tr("Desvinculado"), tr("Cuenta de Strava desvinculada."))
    ...
    def _on_strava_import(self) -> None:
        ...
        if not strava_data or not strava_data.get("access_token"):
            QMessageBox.warning(self, tr("Sin conexión"), tr("Vincula primero tu cuenta de Strava."))
            return

        token, err = _ensure_valid_token(self.profile.config)
        if err:
            QMessageBox.critical(self, tr("Error de autenticación"), err)
            return
        self.manager.save_config(self.profile)
        ...
        if err:
            QMessageBox.critical(self, tr("Error API Strava"), err)
            return
        if not activities:
            QMessageBox.information(self, tr("Sin actividades"),
                tr("No se encontraron actividades de ciclismo en Strava."))
            return
        ...
        if not new_acts:
            QMessageBox.information(self, tr("Al día"),
                f"{tr('Todas las actividades ya están importadas.')} ({len(activities)})")
            return

        if not confirmar(self, tr("Importar desde Strava"),
            f"{len(new_acts)} {tr('actividades nuevas de ciclismo encontradas.')}" + "\n" +
            tr("¿Importar ahora?")):
            return

        # ── threading.Thread + QTimer polling (evita deadlocks de QThread) ──
        self._progress_dlg = QProgressDialog(
            tr("Preparando importación..."), tr("Cancelar"), 0, len(new_acts), self
        )
        self._progress_dlg.setWindowTitle(tr("Importando desde Strava"))
    ...
    def _poll_strava_import(self) -> None:
        ...
        if state["status"] != "running":
            ...
            if skipped:
                parts.append(f"⚠️ {len(skipped)} {tr('duplicadas') if len(skipped) > 1 else tr('duplicada')} ({tr('ya existían como .fit/.tcx')})" )
            if errors:
                parts.append(f"❌ {errors} {tr('errores') if errors > 1 else tr('error')}" )
            if not parts:
                parts.append("No se importaron actividades.")

            QMessageBox.information(self, tr("Strava — Importación"), "\n".join(parts))
    ...
    def _on_language_changed(self, index: int) -> None:
        ...
        base_dir = _os.path.join(_os.path.expanduser("~"), ".ciclometricas")
        _os.makedirs(base_dir, exist_ok=True)
        lang_file = _os.path.join(base_dir, "language.json")
        with open(lang_file, "w", encoding="utf-8") as f:
            json.dump({"language": lang}, f)
        QMessageBox.information(
            self, tr("Idioma"),
            "Language updated. Restart the app to apply." if lang == "en"
            else tr("Idioma actualizado. Reinicia la aplicación para aplicar los cambios."),
        )
    ...
    def _build_backup_tab(self) -> QWidget:
        ...
        card = _make_card(tr("Copia de seguridad"), tr("Guarda o restaura una copia de tu base de datos."))
        inner = card.layout()

        btn_backup = QPushButton("\U0001f4be  " + tr("Crear copia de seguridad"))
        btn_backup.setFixedHeight(44)
        btn_backup.setMinimumWidth(260)
        btn_backup.clicked.connect(self._on_create_backup)
        inner.addWidget(btn_backup)

        btn_restore = QPushButton("\U0001f4c2  " + tr("Restaurar copia de seguridad"))
        btn_restore.setFixedHeight(44)
        btn_restore.setMinimumWidth(260)
        btn_restore.setProperty("class", "ghost")
        btn_restore.clicked.connect(self._on_restore_backup)
        inner.addWidget(btn_restore)
    ...
    def _on_create_backup(self) -> None:
        try:
            ...
            if not os.path.isfile(db_path):
                QMessageBox.warning(self, tr("Error"), tr("No se encontró la base de datos."))
                return
            dest, _ = QFileDialog.getSaveFileName(
                self, tr("Seleccionar destino para la copia"),
                os.path.expanduser(f"~/ciclometricas_backup_{self.profile.name}.db"),
                "SQLite (*.db);;All files (*)",
            )
            if not dest:
                return
            shutil.copy2(db_path, dest)
            QMessageBox.information(self, tr("Copia de seguridad"), tr("Copia de seguridad creada correctamente."))
        except Exception as exc:
            QMessageBox.critical(self, tr("Error"), f"{tr('No se pudo crear la copia de seguridad.')}\n{exc}")
    ...
    def _on_restore_backup(self) -> None:
        try:
            ...
            src, _ = QFileDialog.getOpenFileName(
                self, tr("Seleccionar archivo de copia"),
                os.path.expanduser("~"), "SQLite (*.db);;All files (*)",
            )
            if not src:
                return
            if not confirmar(self, tr("Restaurar copia"),
                tr("¿Restaurar esta copia de seguridad?\nEsto reemplazará todos tus datos actuales.")):
                return
            db_path = str(self.profile.db_path)
            try:
            ...
            QMessageBox.information(self, tr("Restaurar copia"),
                tr("Copia restaurada correctamente. Reinicia la aplicación."))
        except Exception as exc:
            QMessageBox.critical(self, tr("Error"), f"{tr('No se pudo restaurar la copia.')}\n{exc}")

    # ============================================================
    # Tab 7: IA (Consejero IA — Ollama)
    # ============================================================

    def _build_ia_tab(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(0, 14, 0, 0)
        layout.setSpacing(18)
        ...
        # --- Tarjeta URL ---
        card_url = _make_card(
            tr("Conexión con Ollama"),
            tr("URL del servidor Ollama local."),
        )
        inner_url = card_url.layout()

        url_row = QHBoxLayout()
        lbl_url = QLabel(tr("URL:"))
        lbl_url.setStyleSheet(f"font-size: {FONT_SIZE_BASE}; color: {COLORS['fg']}; background: transparent;")
        url_row.addWidget(lbl_url)

        self._ia_url_input = QLineEdit()
        self._ia_url_input.setText(DEFAULT_OLLAMA_URL)
        self._ia_url_input.setPlaceholderText("http://localhost:11434")
        self._ia_url_input.setMinimumWidth(300)
        url_row.addWidget(self._ia_url_input, 1)

        btn_test = QPushButton(tr("🔌 Probar conexión"))
        btn_test.setFixedHeight(36)
        btn_test.clicked.connect(self._on_test_ollama)
        url_row.addWidget(btn_test)

        inner_url.addLayout(url_row)

        self._ia_status_label = QLabel("")
        self._ia_status_label.setStyleSheet(
            f"font-size: {FONT_SIZE_SM}; color: {COLORS['fg_muted']}; background: transparent;"
        )
        inner_url.addWidget(self._ia_status_label)

        layout.addWidget(card_url)

        # --- Tarjeta Modelo ---
        card_model = _make_card(
            tr("Modelo de IA"),
            tr("Selecciona el modelo instalado en Ollama que usará el Consejero IA."),
        )
        inner_model = card_model.layout()

        model_row = QHBoxLayout()
        lbl_model = QLabel(tr("Modelo:"))
        lbl_model.setStyleSheet(f"font-size: {FONT_SIZE_BASE}; color: {COLORS['fg']}; background: transparent;")
        model_row.addWidget(lbl_model)

        self._ia_model_combo = QComboBox()
        self._ia_model_combo.setMinimumWidth(250)
        self._ia_model_combo.addItem(DEFAULT_MODEL)
        model_row.addWidget(self._ia_model_combo, 1)

        btn_refresh = QPushButton(tr("🔄 Actualizar lista"))
        ...
        inner_model.addLayout(model_row)
        layout.addWidget(card_model)

        # --- Botón guardar ---
        btn_save = QPushButton(tr("💾 Guardar configuración IA"))
        btn_save.setFixedHeight(44)
        btn_save.setMinimumWidth(260)
        btn_save.clicked.connect(self._on_save_ia_config)
        layout.addWidget(btn_save)

        # --- Info ---
        info = QLabel(
            tr("ℹ️ Ollama debe estar instalado y ejecutándose en tu equipo. ")
            + tr("Descárgalo en ") + '<a style="color: ' + COLORS['accent'] + '" '
            'href="https://ollama.com">ollama.com</a>. '
            + tr("La comunicación es 100 % local, sin internet.")
        )
    ...
    def _load_ia_config(self) -> None:
        """Carga la configuración IA desde el perfil del atleta."""
        url = self.profile.config.get("ollama_url", DEFAULT_OLLAMA_URL)
        model = self.profile.config.get("ollama_model", DEFAULT_MODEL)
        self._ia_url_input.setText(url)
        # Asegurar que el modelo está en el combo
        idx = self._ia_model_combo.findText(model)
        if idx < 0:
            self._ia_model_combo.addItem(model)
            idx = self._ia_model_combo.findText(model)
        self._ia_model_combo.setCurrentIndex(idx)

    def _on_test_ollama(self) -> None:
        """Prueba la conexión con Ollama."""
        url = self._ia_url_input.text().strip()
        if not url:
            self._ia_status_label.setText(tr("⚠️ Introduce una URL válida."))
            return
        coach = get_ai_coach(ollama_url=url)
        ok = coach.check_connection()
        if ok:
            models = coach.list_models()
            names = [m.name for m in models]
            self._ia_status_label.setText(
                f"✅ {tr('Conectado')} — {len(models)} {tr('modelo(s) detectado(s)')}"
            )
            self._ia_status_label.setStyleSheet(
                f"font-size: {FONT_SIZE_SM}; color: {COLORS['success']}; background: transparent;"
            )
            # Actualizar combo de modelos
            current = self._ia_model_combo.currentText()
            self._ia_model_combo.clear()
            for n in names:
                self._ia_model_combo.addItem(n)
            idx = self._ia_model_combo.findText(current)
            if idx >= 0:
                self._ia_model_combo.setCurrentIndex(idx)
        else:
            self._ia_status_label.setText(
                f"❌ {tr('No se pudo conectar con Ollama en')} {url}"
            )
            self._ia_status_label.setStyleSheet(
    ...
    def _on_refresh_models(self) -> None:
        ...
        self._on_test_ollama()

    def _on_save_ia_config(self) -> None:
        """Guarda la configuración IA en el perfil."""
        url = self._ia_url_input.text().strip() or DEFAULT_OLLAMA_URL
        model = self._ia_model_combo.currentText() or DEFAULT_MODEL

        self.profile.config["ollama_url"] = url
        self.profile.config["ollama_model"] = model
        self.manager.save_config()

        # Actualizar instancia del coach
        coach = get_ai_coach(ollama_url=url, model=model)

        QMessageBox.information(
            self,
            tr("Configuración IA"),
            tr("Configuración del Consejero IA guardada correctamente."),
        )

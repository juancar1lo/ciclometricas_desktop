...
class ImportView(QWidget):
    ...
    request_show_all = Signal()       # Pide a la vista de actividades cambiar filtro a tr("Todo")
    ...
    def __init__(self, profile: AthleteProfile, parent=None):
        ...
        # Título
        title = QLabel(f"📥  {tr('Importar archivos')}")
        title.setStyleSheet(
            f"font-size: {FONT_SIZE_TITLE}; font-weight: bold; color: {COLORS['fg']};"
        )
        layout.addWidget(title)

        desc = QLabel(tr("Selecciona archivos .FIT o .TCX exportados de tu ciclocomputador o Strava."))
        ...
        # Botón seleccionar
        btn_row = QHBoxLayout()
        self.btn_select = QPushButton(tr("\U0001F4C2  Seleccionar archivos"))
    ...
    def _open_file_dialog(self) -> None:
        files, _ = QFileDialog.getOpenFileNames(
            self,
            tr("Seleccionar archivos de actividad"),
            str(Path.home()),
    ...
    def _on_files_selected(self, paths: List[Path]) -> None:
        ...
        self.progress_label.setText(f"{tr('Importando')} 0/{len(paths)}...")
    ...
    def _on_progress(self, index: int, result: ImportResult) -> None:
        ...
        self.progress_label.setText(f"{tr('Importando')} {index + 1}/{total}...")
    ...
    def _on_finished(self, results: List[ImportResult]) -> None:
        ...
        errors = sum(1 for r in results if r.status == tr("error"))

        parts = []
        if created:
            parts.append(f"\u2705 {created} {tr('importados') if created > 1 else tr('importado')}")
        if dupes:
            parts.append(f"\u26a0\ufe0f {dupes} {tr('duplicados') if dupes > 1 else tr('duplicado')}")
        if errors:
        ...
        if created > 0:
            ...
            # que la vista de actividades cambie el filtro a tr("Todo") para que
...
class _DropZone(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        text = QLabel(tr("Arrastra archivos .FIT o .TCX aquí"))

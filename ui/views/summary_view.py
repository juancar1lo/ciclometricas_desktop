...
_MONTH_FULL = {
    1: tr("Enero"), 2: tr("Febrero"), 3: tr("Marzo"), 4: tr("Abril"),
    5: tr("Mayo"), 6: tr("Junio"), 7: tr("Julio"), 8: tr("Agosto"),
    9: tr("Septiembre"), 10: tr("Octubre"), 11: tr("Noviembre"), 12: tr("Diciembre"),
}
...
class ZoneMiniBarDelegate(QStyledItemDelegate):
    ...
    def helpEvent(self, event, view, option, index):
        ...
        if zones and event.type() == QEvent.Type.ToolTip:
            total = sum(zones.get(k, 0) for k in _ZONE_KEYS)
            if total > 0:
                lines = [tr("Distribución por zonas:")]
                for k in _ZONE_KEYS:
                    secs = zones.get(k, 0)
                    if secs < 30:
                        continue
...
class SummaryView(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        lay.addWidget(_section_title(
            "📈", tr("Resumen de entrenamiento"),
            tr("Agregados semanales y mensuales con tendencias.")
        ))
        ...
        btn_pdf = QPushButton(f"📥  {tr('Descargar PDF')}")
        ...
        # Weekly table
        self._tbl_weekly = self._make_table(tr("Semana"))
        self._tabs.addTab(self._tbl_weekly, tr("📅 Semanal"))

        # Monthly table
        self._tbl_monthly = self._make_table(tr("Mes"))
        self._tabs.addTab(self._tbl_monthly, tr("📊 Mensual"))
        ...
        lay.addWidget(_section_title(
            "🔄", tr("Evolución de zonas Coggan"),
            tr("Distribución porcentual del tiempo por zona de potencia.")
        ))
    ...
    def _make_table(self, first_col: str) -> QTableWidget:
        ...
        tbl.setHorizontalHeaderLabels([
            first_col, tr("Entrenos"), tr("Horas"), "Km", tr("Desnivel"),
            tr("TSS"), "kJ", "IF med", tr("Zonas"),
        ])
    ...
    def _update_sparklines(self) -> None:
        ...
        for title, data, color in [
            (tr("TSS / semana"), w_tss, "#FF6B35"),
            (tr("Horas / semana"), w_hrs, "#22D3EE"),
            (tr("TSS / mes"), m_tss, "#FF6B35"),
            (tr("Horas / mes"), m_hrs, "#22D3EE"),
        ]:
    ...
    def _draw_zones_chart(self, _idx: int = 0) -> None:
        ...
        if not rows:
            empty_txt = pg.TextItem(
                tr("Sin datos para el periodo seleccionado"),
    ...
    def _export_pdf(self) -> None:
        ...
        try:
            ...
            QMessageBox.critical(self, tr("Error"), f"{tr('Error al generar PDF')}:\n{tb[:600]}")
    ...
    def _export_pdf_impl(self) -> None:
        ...
        if not rows:
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.information(self, tr("Sin datos"), tr("No hay datos para exportar."))
            return
        ...
        kind = tr("semanal") if is_weekly else tr("mensual")
        today = date.today()
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, tr("Guardar PDF"), default_name,
            tr("Archivos PDF (*.pdf);;Todos los archivos (*)"),
        )
        if not path:
            return
        ...
        kpi_row1 = (
            "<tr>"
            + _kpi(tr("Entrenamientos"), str(total_acts), "", trend_acts)
            + _kpi(tr("Horas totales"), f"{total_hours:.1f}", "h", trend_hrs)
            + _kpi(tr("Distancia"), f"{total_km:.0f}", "km", trend_km)
            + "</tr>"
        )
        kpi_row2 = (
            "<tr>"
            + _kpi(tr("Desnivel"), f"{total_elev:,.0f}".replace(",", "."), "m", trend_elev)
            + _kpi(tr("TSS total"), f"{total_tss:.0f}", "", trend_tss)
            + _kpi(tr("Trabajo"), f"{total_kj:,.0f}".replace(",", "."), "kJ", trend_kj)
            + "</tr>"
        )
        ...
        col_label = tr("Semana") if is_weekly else tr("Mes")
        ...
        if self._all_activities:
            ...
            for a in sorted_acts[:10]:
                ...
                name = (getattr(a, 'custom_name', None)
                        or getattr(a, 'file_name', '') or tr("Actividad"))
                if len(name) > 30:
                    name = name[:27] + "..."
        ...
        if ftp_val:
            ...
            if weight_val:
                ftp_line += f"&nbsp;&nbsp;·&nbsp;&nbsp;{tr('Peso')}: {weight_val:.0f} kg"
        ...
        if os.path.exists(path):
            ...
            QMessageBox.information(
                self, tr("PDF generado"),
                f"{tr('Informe guardado correctamente:')}\n{path}",
            )

...
class _ClickableTable(QTableWidget):
    """QTableWidget with robust click signals via event override."""
    row_activated = Signal(int)  # visual row

    def mouseDoubleClickEvent(self, event):
        idx = self.indexAt(event.pos())
        if idx.isValid() and idx.column() < 10:
            self.row_activated.emit(idx.row())
...
class ActivitiesView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Título
        title = QLabel(tr("Entrenamientos"))
        title.setStyleSheet(
        ...
        self._search.setPlaceholderText(tr("Buscar por nombre, archivo o deporte..."))
        ...
        # Botón sesión manual
        self._manual_btn = QPushButton(tr("➕  Sesión manual"))
        ...
        for label, _days in _period_options():
            self._period_combo.addItem(label)
        self._period_combo.setCurrentIndex(2)  # tr("Últimos 90 días")
        ...
        # Primary: custom signal from mouse event overrides (most reliable)
        self.table.row_activated.connect(self._on_row_activated)
        # Fallback: standard Qt signals
        self.table.doubleClicked.connect(self._on_index_click)
        self.table.clicked.connect(self._on_index_click)
        ...
        _fc_max = tr("FC máx")
        columns = [
            tr("Fecha"), tr("Nombre"), tr("Duración"), f"{_dist}\n(km)",
            _pot, tr("NP"), "IF", tr("TSS"),
            _fc, _fc_max, "", "",
        ]
    ...
    def show_all(self) -> None:
        ...
        todo_idx = next(
            (i for i, (_, days) in enumerate(_period_options()) if days == 0), len(_period_options()) - 1
        )
    ...
    def _apply_filters(self, *_args) -> None:
        ...
        _label, days = _period_options()[idx]
        if days > 0:
        ...
        self.count_label.setText(f"{len(activities)} {tr('actividades en el periodo seleccionado.')}")
    ...
    def _populate_table(self) -> None:
        ...
        for row, act in enumerate(self._filtered):
            ...
            rename_btn.setToolTip(tr("Renombrar actividad"))
            ...
            del_btn.setToolTip(tr("Eliminar actividad"))
    ...
    def _open_row(self, row: int) -> None:
        """Core: abre detalle de la actividad en la fila visual dada."""
        try:
            # Method 1: UserRole (sort-safe)
            name_item = self.table.item(row, 1)
            if name_item is not None:
                act_id = name_item.data(Qt.ItemDataRole.UserRole)
                if act_id is not None:
                    print(f"[Activities] Opening activity id={act_id} via UserRole")
                    self.open_detail.emit(int(act_id))
                    return
            # Method 2: filtered list (v2 approach)
            if 0 <= row < len(self._filtered):
                act = self._filtered[row]
                print(f"[Activities] Opening activity id={act.id} via _filtered[{row}]")
                self.open_detail.emit(int(act.id))
                return
            print(f"[Activities] WARNING: row={row} out of range, filtered={len(self._filtered)}")
        except Exception as e:
            print(f"[Activities] ERROR in _open_row: {e}")
            import traceback; traceback.print_exc()

    def _on_row_activated(self, row: int) -> None:
        """Handler for custom row_activated signal (from mouse events)."""
        print(f"[Activities] row_activated signal: row={row}")
        self._open_row(row)

    def _on_index_click(self, index) -> None:
        """Handler for standard clicked/doubleClicked signals."""
        row = index.row()
        col = index.column()
        print(f"[Activities] index_click signal: row={row}, col={col}")
        if col >= 10:
            return
        self._open_row(row)
    ...
    def _delete_activity(self, activity_id: int) -> None:
        ...
        if not confirmar(
            self,
            tr("Eliminar actividad"),
            tr("¿Estás seguro de que quieres eliminar esta actividad?") + "\n" +

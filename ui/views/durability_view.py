...
class DurabilityView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._zone_ref_label: str = tr("CP")
    ...
    def _build_ui(self) -> None:
        ...
        lay.addWidget(_section_title(
            "🧪", tr("Test de Durabilidad"),
            tr("DRI — Durability Index · Mide la degradación de CP bajo fatiga acumulada")
        ))
        ...
        # CP overlay chart
        lay.addWidget(_section_title("📈", tr("Curvas CP: Fresca vs Fatigada"),
            tr("Superposición de la curva de potencia fresca y fatigada")))
        self._cp_chart = make_plot(height=300)
        lay.addWidget(self._cp_chart)

        # Zonas Coggan: fresca vs fatigada
        lay.addWidget(_section_title("📊", tr("Zonas Coggan: Fresca vs Fatigada"),
            tr("Rangos de vatios por zona según referencia fresca y fatigada")))
        ...
        # Área entre curvas
        lay.addWidget(_section_title("📐", tr("Área entre curvas"),
            tr("Capacidad perdida entre 30s y 30min de duración")))
        self._area_cards_row = QHBoxLayout()
        self._area_cards_row.setSpacing(10)
        lay.addLayout(self._area_cards_row)

        # Decay curve chart
        lay.addWidget(_section_title("📉", tr("Curva de Decaimiento Exponencial"),
            tr("DRI(kJ) = 100 × e^(-λ × kJ) — Modelo de degradación")))
        ...
        # History table
        lay.addWidget(_section_title("📋", tr("Historial de tests")))
    ...
    def _build_form(self) -> QFrame:
        ...
        title = QLabel(tr("➕  Registrar nuevo test"))
        ...
        # kJ consumed
        form_grid.addWidget(QLabel(tr("kJ acumulados:")), 0, 0)
        self._kj_input = QLineEdit()
        self._kj_input.setPlaceholderText(tr("ej: 2000"))
        ...
        # Power 3 min
        form_grid.addWidget(QLabel(tr("Potencia 3 min (W):")), 1, 0)
        self._p3_input = QLineEdit()
        self._p3_input.setPlaceholderText(tr("ej: 310"))
        self._p3_input.setValidator(QIntValidator(50, 2000))
        form_grid.addWidget(self._p3_input, 1, 1)

        # Power 12 min
        form_grid.addWidget(QLabel(tr("Potencia 12 min (W):")), 2, 0)
        self._p12_input = QLineEdit()
        self._p12_input.setPlaceholderText(tr("ej: 260"))
        self._p12_input.setValidator(QIntValidator(50, 2000))
        form_grid.addWidget(self._p12_input, 2, 1)

        # Notes
        form_grid.addWidget(QLabel(tr("Notas (opcional):")), 3, 0)
        self._notes_input = QLineEdit()
        self._notes_input.setPlaceholderText(tr("Detalles del test..."))
        form_grid.addWidget(self._notes_input, 3, 1, 1, 2)

        lay.addLayout(form_grid)

        # Save button
        btn_save = QPushButton(tr("💾  Guardar test"))
    ...
    def _make_table(self) -> QTableWidget:
        ...
        tbl.setHorizontalHeaderLabels([
            tr("Fecha"), "kJ", "P3min", "P12min", "CP fat.", "DRI %", tr("Estado"), "",
        ])
    ...
    def _make_zones_table(self) -> QTableWidget:
        ...
        tbl.setHorizontalHeaderLabels([tr("Zona"), tr("Fresca (W)"), tr("Fatigada (W)"), "Δ W"])
        tbl.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        tbl.verticalHeader().setVisible(False)
        tbl.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        tbl.setSelectionMode(QAbstractItemView.SelectionMode.NoSelection)
    ...
    def _make_explanation(self) -> QFrame:
        ...
        title = QLabel(tr("¿Cómo funciona?"))
        title.setStyleSheet(f"font-size: 14px; font-weight: 700; color: {COLORS['fg']}; border: none;")
        lay.addWidget(title)

        texts = [
            (tr("El <b>Test de Durabilidad</b> mide tu capacidad de mantener la potencia crítica ") +
             tr("después de acumular trabajo mecánico (kJ). El protocolo es:")),
            "",
            tr("1. Acumula un número conocido de kJ sobre la bici (ej: 2500 kJ)."),
            '2. Haz un test all-out de <b>3 minutos</b> seguido de uno de <b>12 minutos</b>.',
            tr("3. Registra los resultados aquí."),
            "",
            (tr("La app calcula el <b>CP fatigado</b> con el modelo de 2 parámetros y lo compara ") +
             tr("con tu CP fresco para obtener el <b>Índice de Durabilidad (DRI)</b>:")),
            "",
            '<span style="font-family:monospace;">DRI = (CP<sub>fatigado</sub> / CP<sub>fresco</sub>) × 100</span>',
            "",
            tr("<b>Clasificación del DRI:</b>"),
            f'• <span style="color:#22c55e"><b>≥ 95%</b></span> — {tr("Excelente durabilidad")}',
            f'• <span style="color:#3b82f6"><b>92 – 94,99%</b></span> — {tr("Buena durabilidad")}',
            f'• <span style="color:#eab308"><b>88 – 91,99%</b></span> — {tr("Mejorable")}',
            f'• <span style="color:#ef4444"><b>&lt; 88%</b></span> — {tr("Limitante en recorridos largos")}',
            "",
            (tr("Con múltiples tests a distintos kJ, se ajusta un <b>modelo exponencial</b> de decaimiento ") +
             tr("que permite <b>extrapolar</b> tu DRI a valores de kJ que no hayas testeado directamente.")),
        ]
        for t in texts:
            lbl = QLabel(t)
    ...
    def _save_test(self) -> None:
        try:
            ...
            QMessageBox.warning(self, tr("Datos inválidos"), tr("Rellena todos los campos numéricos."))
            return

        if self._fresh_cp <= 0:
            QMessageBox.warning(self, tr("Sin referencia"),
                tr("No hay modelo CP fresco disponible. Registra primero un test de potencia en Configuración."))
            return

        if p3 <= p12:
            QMessageBox.warning(self, tr("Error"), tr("La potencia de 3 min debe ser mayor que la de 12 min."))
            return

        result = calc_durability_index(p3, p12, self._fresh_cp, self._fresh_w)
        if result is None:
            QMessageBox.warning(self, tr("Error"), tr("No se pudo calcular el DRI con estos datos."))
            return
    ...
    def _delete_test(self, test_id: int) -> None:
        if not confirmar(self, tr("Eliminar test"), tr("¿Eliminar este test de durabilidad?")):
            return
        with get_session() as session:
    ...
    def _update_cards(self) -> None:
        ...
        if self._tests:
            ...
            self._cards_row.addWidget(_stat_card(
                tr("Último DRI"), "—", COLORS["fg_dim"], tr("Sin tests aún"),
            ))
    ...
    def _draw_cp_chart(self) -> None:
        ...
        if not self._tests or self._fresh_cp <= 0:
            txt = pg.TextItem(tr("Registra un test para ver la comparación de curvas CP"),
        ...
        for clr, lbl_text, y_base, direction in [
            ("#22c55e", f"{tr('CP fresco')}: {self._fresh_cp:.0f}W",
             self._fresh_cp, 1),
            ("#ef4444", f"{tr('CP fatigado')}: {last.cp_fatigued:.0f}W",
             last.cp_fatigued, -1),
        ]:
        ...
        legend.addItem(fresh_line, tr("CP Fresco"))
        legend.addItem(fat_line, tr("CP Fatigado"))

        pw.setLabel("bottom", tr("Duración (s)"))
        pw.setLabel("left", tr("Potencia (W)"))
        ...
        def _fmt_cp(xv, yv):
            ...
            lines = [
                tooltip_header(f"{tr('Duración')}: {dur_str}"),
                tooltip_line(tr("P. Fresca"), f"{p_fresh:.0f} W", "#22c55e"),
                tooltip_line(tr("P. Fatigada"), f"{p_fat:.0f} W ({last_kj:.0f} kJ)", "#ef4444"),
                tooltip_line(tr("Diferencia"), f"–{diff:.0f} W ({diff / max(p_fresh, 1) * 100:.1f}%)"),
            ]
    ...
    def _draw_zones_chart(self) -> None:
        ...
        if not self._tests or self._fresh_cp <= 0:
            txt = pg.TextItem(tr("Registra un test para ver la comparación de zonas"),
                color=QColor(COLORS["fg_muted"]))
            txt.setFont(QFont("Segoe UI", 10))
            pw.addItem(txt)
            txt.setPos(1, 200)
        ...
        for z in POWER_ZONES:
            ...
            zone_data.append({
                "label": tr(z.label), "short": z.short_label, "color": z.color,
        ...
        legend.addItem(dummy_fresh, f"{tr('CP Fresca')} ({fresh_ref}W)")
        legend.addItem(dummy_fat, f"{tr('CP Fatigada')} ({fat_ref}W)")
        ...
        def _fmt_zones(xv, _yv):
            ...
            lines = [
                ...
                tooltip_line(tr("Fresca"), f"{z['f_min']}–{z['f_max']} W", "#22d3ee"),
                tooltip_line(tr("Fatigada"), f"{z['fa_min']}–{z['fa_max']} W", "#f97316"),
                tooltip_line(tr("Δ mín"), f"−{z['f_min'] - z['fa_min']} W", "#ef4444"),
                tooltip_line(tr("Δ máx"), f"−{z['f_max'] - z['fa_max']} W", "#ef4444"),
            ]
            return tooltip_html(lines)
    ...
    def _update_area_cards(self) -> None:
        ...
        if not self._tests or self._fresh_cp <= 0:
            ...
            self._area_cards_row.addWidget(_stat_card(
                tr("Área total"), "—", COLORS["fg_dim"], reason,
            ))
            return
    ...
    def _draw_decay_chart(self) -> None:
        ...
        if not self._tests:
            txt = pg.TextItem(tr("Registra tests a diferentes kJ para ver el modelo de decaimiento"),
                color=QColor(COLORS["fg_muted"]))
            txt.setFont(QFont("Segoe UI", 10))
            pw.addItem(txt)
            txt.setPos(500, 95)
        ...
        pw.setYRange(70, 105)
        pw.setLabel("bottom", tr("kJ acumulados"))
        ...
        def _fmt(xv, yv):
            ...
            if near_test:
                ...
                lines = [
                    ...
                    tooltip_line(tr("DRI"), f"{t.dri_percent:.1f}% — {lbl}", clr),
                    tooltip_line(tr("CP fatigado"), f"{t.cp_fatigued:.0f} W"),
                    tooltip_line(tr("P3min / P12min"), f"{t.power_3min}W / {t.power_12min}W"),
                ]
                if t.notes:
                ...
                lines = [
                    tooltip_header(f"{tr('Modelo')} @ {kj_val:.0f} kJ"),
                    tooltip_line(tr("DRI estimado"), f"{dri_pred:.1f}%", clr),
                    tooltip_line(tr("Estado"), lbl),
                ]
                return tooltip_html(lines)
            return ""

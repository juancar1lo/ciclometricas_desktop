...
class ReadinessView(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        lay.addWidget(_section_title(
            "🎯", tr("Preparación para Competir"),
            tr("Race Readiness Score · Índice compuesto de tu estado actual")
        ))
        ...
        gc_title = QLabel(tr("Race Readiness Score"))
        ...
        gc_sub = QLabel(tr("Puntuación compuesta 0–100"))
    ...
    def _make_explanation(self) -> QFrame:
        ...
        texts = [
            tr("El <b>Race Readiness Score</b> combina tres dimensiones clave del entrenamiento:"),
            tr("• <b>Forma (40%)</b> — Tu TSB (Training Stress Balance). El punto óptimo es +5 a +15."),
            tr("• <b>Fitness (35%)</b> — Tu CTL relativo a tu máximo histórico, más bonus/penalización por tendencia (ramp rate)."),
            tr("• <b>Variabilidad (25%)</b> — Inverso de la monotonía. Mayor variedad diaria = mejor score."),
            tr("🟢 ≥75 = Listo para competir · 🟡 50–74 = Casi listo · 🔴 <50 = No es el momento"),
        ]
        for t in texts:
            lbl = QLabel(t)
    ...
    def _refresh_data(self) -> None:
        ...
        self._advice_label.setText(f"💡 {tr('Consejo')}: {rrs.advice}")
        ...
        factors = [
            (tr("Forma (TSB)"), "40%", rrs.form_score),
            (tr("Fitness (CTL)"), "35%", rrs.fitness_score),
            (tr("Variabilidad"), "25%", rrs.variability_score),
        ]
        ...
        self._metrics_container.addWidget(self._make_metric_row(
            ...
            f"{tr('Máximo histórico')}: {ctl_max:.1f}",
        ))
        ramp_str = f"{'+' if ramp and ramp > 0 else ''}{ramp:.1f} TSS/d" if ramp is not None else "N/A"
        self._metrics_container.addWidget(self._make_metric_row(
            "Ramp Rate", ramp_str,
            tr("Velocidad de cambio del CTL semanal"),
        ))
        mon_str = f"{monotony:.2f}" if monotony is not None else "N/A"
        self._metrics_container.addWidget(self._make_metric_row(
            tr("Monotonía"), mon_str,
            tr("Última semana (ideal < 1.5)"),
        ))

    def _show_no_data(self) -> None:
        self._gauge.set_score(0, "not_ready")
        self._status_label.setText(tr("Sin datos suficientes"))
        self._advice_label.setText(tr("Importa actividades para calcular tu preparación."))

...
def _format_recovery_time(hours: Optional[int]) -> str:
    if hours is None:
        return tr("¡Ya estás fresco!")
    if hours < 1:
        return tr("Menos de 1 hora")
    if hours < 24:
        return f"~{hours}h"
    days = hours // 24
    remain_h = hours % 24
    if remain_h == 0:
        return f"{days} {tr('día') if days == 1 else tr('días')}"
...
class RecoveryView(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        lay.addWidget(_section_title(
            "🔋", tr("Recuperación Estimada"),
            tr("Proyección de tu estado de recuperación basada en el modelo CTL/ATL/TSB")
        ))
        ...
        # Projection chart
        lay.addWidget(_section_title("📉", tr("Proyección de TSB (próximos 14 días)"),
            tr("Simulación asumiendo descanso completo. La línea verde marca TSB = +5 (fresco)")))
        self._proj_chart = make_plot(height=280)
        lay.addWidget(self._proj_chart)

        # Activity recovery table
        lay.addWidget(_section_title("📋", tr("Recuperación por Actividad"),
            tr("Tiempo de recuperación estimado para las últimas sesiones")))
    ...
    def _make_table(self) -> QTableWidget:
        ...
        tbl.setHorizontalHeaderLabels([
            tr("Actividad"), tr("Fecha"), tr("TSS"), tr("Duración"), tr("Recuperación"),
        ])
    ...
    def _make_explanation(self) -> QFrame:
        ...
        texts = [
            (tr("La <b>Recuperación Estimada</b> usa el modelo Banister de CTL/ATL/TSB. ") +
             tr("Proyecta hacia adelante asumiendo descanso completo (TSS = 0 cada día):")),
            tr("• <b>ATL (fatiga aguda)</b> decae con constante τ = 7 días → baja rápidamente sin carga"),
            tr("• <b>CTL (fitness crónico)</b> decae con τ = 42 días → se mantiene más estable"),
            tr("• Como ATL baja más rápido que CTL, el <b>TSB sube</b> hasta cruzar el umbral de frescura (+5)"),
            "",
            tr("🟢 TSB ≥ +5 = Fresco · 🟡 -10 a +5 = Recuperando · 🔴 < -10 = Fatigado"),
            "",
            tr("La recuperación por actividad estima el impacto individual de cada sesión sobre tu estado previo."),
        ]
        for t in texts:
            lbl = QLabel(t)
    ...
    def _refresh_data(self) -> None:
        ...
        self._metrics_container.addWidget(self._make_metric_row(
            ...
            tr("Carga crónica de entrenamiento"), "#60a5fa",
        ))
        self._metrics_container.addWidget(self._make_metric_row(
            "🏃", "ATL (Fatiga)", f"{recovery.current_atl:.1f}",
            tr("Carga aguda de entrenamiento"), "#f97316",
        ))
        ...
        self._metrics_container.addWidget(self._make_metric_row(
            "📈", "TSB (Forma)", tsb_str,
            tr("Balance de estrés del entrenamiento"), tsb_color,
        ))
        self._metrics_container.addWidget(self._make_metric_row(
            "⏰", tr("Tiempo hasta recuperación"),
            _format_recovery_time(recovery.hours_to_recovery),
            tr("Asumiendo descanso completo (TSS=0)") if recovery.hours_to_recovery else tr("Listo para entrenar a tope"),
            "#a855f7",
        ))
    ...
    def _show_no_data(self) -> None:
        self._gauge.set_data("fresh", "❓", tr("Sin datos"), None, 0)
        self._advice_label.setText(tr("Importa actividades para calcular tu recuperación."))
    ...
    def _draw_projection(self, projection) -> None:
        ...
        pw.setLabel("left", tr("TSB"))

        # Legend
        for i, (clr, lbl) in enumerate([
            ("#22c55e", tr("TSB")), ("#ff9149", tr("CTL")), ("#ff6363", tr("ATL")),
        ]):
        ...
        def _fmt(xv, yv):
            ...
            p = proj_ref[idx]
            lines = [
                tooltip_header(f"{tr('Día')} +{p.day} ({p.date})"),
                tooltip_line(tr("CTL"), f"{p.ctl:.1f}", "#ff9149"),
                tooltip_line(tr("ATL"), f"{p.atl:.1f}", "#ff6363"),
                tooltip_line(tr("TSB"), f"{p.tsb:+.1f}", "#22c55e"),
            ]

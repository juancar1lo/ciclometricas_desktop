...
# calc.pdc_fatigue no se usa directamente; PDC se calcula desde MMP global
...
# PDC: 5s – hasta la duración máxima de los recorridos
_PDC_X_TICKS: list[int] = [
...
def _period_days(label: str) -> int:
    _MAP = {
        tr("Últimos 7 días"): 7, tr("Últimos 14 días"): 14,
        tr("Últimos 30 días"): 30, tr("Últimos 60 días"): 60,
        tr("Últimos 90 días"): 90, tr("Últimos 180 días"): 180,
        tr("Últimos 365 días"): 365, tr("Todo el histórico"): 9999,
    }
...
def _forecast_days(label: str) -> int:
    ...
    if tr("+7 días predicción") in label:
        return 7
    if tr("+14 días predicción") in label:
        return 14
    return 0
...
def _period_options():
    return [
        tr("Últimos 7 días"), tr("Últimos 14 días"), tr("Últimos 30 días"),
        tr("Últimos 60 días"), tr("Últimos 90 días"), tr("Últimos 180 días"),
        tr("Últimos 365 días"), tr("Todo el histórico"),
        tr("── Predicción ──"),
        tr("90 d +7 días predicción"), tr("90 d +14 días predicción"),
        tr("── Rango ──"),
...
def _month_short():
    return {
        1: tr("ene"), 2: tr("feb"), 3: tr("mar"), 4: tr("abr"), 5: tr("may"), 6: tr("jun"),
        7: tr("jul"), 8: tr("ago"), 9: tr("sept"), 10: tr("oct"), 11: tr("nov"), 12: tr("dic"),
    }
...
def _tsb_state(tsb: float) -> Tuple[str, str, str]:
    """(label, color, hint)."""
    if tsb >= 25:
        return tr("Transición"), COLORS["fg_dim"], tr("Puede que estés perdiendo forma.")
    if tsb >= 5:
        return tr("Fresco"), COLORS["success"], tr("Buen estado de forma para competir.")
    if tsb >= -10:
        return tr("Productivo"), COLORS["accent"], tr("Entrenamiento productivo.")
    if tsb >= -30:
        return tr("Óptimo"), "#FF9149", tr("Carga óptima para mejorar.")
    return tr("Alto riesgo"), COLORS["destructive"], tr("Cuidado con el sobreentrenamiento.")


def _ramp_hint(rr: Optional[float]) -> Tuple[str, str]:
    if rr is None:
        return tr("Sin datos"), COLORS["fg_dim"]
    if rr > 1:
        return tr("Incrementas carga."), COLORS["success"]
    if rr > -1:
        return tr("Carga estable."), COLORS["accent"]
    return tr("Pierdes forma; valora cargar más."), COLORS["warning"]


def _acwr_state(acwr: float) -> Tuple[str, str]:
    if acwr < 0.8:
        return tr("Infracarga"), "#60B5FF"
    if acwr <= 1.3:
        return tr("Óptimo"), COLORS["success"]
    if acwr <= 1.5:
        return tr("Alto"), COLORS["warning"]
...
class DashboardView(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        header = QHBoxLayout()
        title = QLabel(tr("Panel"))
        title.setProperty("class", "title")
        header.addWidget(title)
        header.addStretch()

        self._period_combo = QComboBox()
        self._period_combo.addItems(_period_options())
        # Deshabilitar separadores
        model = self._period_combo.model()
        for i, opt in enumerate(_period_options()):
            if opt.startswith("──"):
                item = model.item(i)
                if item:
        ...
        desc = QLabel(tr("Visión general de tu carga, forma y zonas de entrenamiento."))
    ...
    def _date_label(ts: float) -> str:
        ...
        try:
            ...
            ms = {1: tr("ene"), 2: tr("feb"), 3: tr("mar"), 4: tr("abr"), 5: tr("may"), 6: tr("jun"),
                  7: tr("jul"), 8: tr("ago"), 9: tr("sept"), 10: tr("oct"), 11: tr("nov"), 12: tr("dic")}
    ...
    def _build_resumen_tab(self) -> None:
        ...
        self._tabs.addTab(scroll, tr("◉ Resumen"))

        # Botón subir entrenamiento
        btn_row = QHBoxLayout()
        btn_row.addStretch()
        btn_import = QPushButton(f"📤  {tr('Subir entrenamiento')}")
    ...
    def _update_resumen(self) -> None:
        ...
        sc_ctl = StatCard(
            "📈", tr("CTL (FORMA CRÓNICA)"), ctl_val, tr("CTL"),
            hint=tr("Media exponencial de TSS · 42 d"),
            accent="#FF9149",
        )
        ...
        sc_atl = StatCard(
            "🔥", tr("ATL (FATIGA)"), atl_val, tr("ATL"),
            hint=tr("Media exponencial de TSS · 7 d"),
        ...
        if rr is not None:
            ...
            rr_label = tr("Creciente") if rr > 1 else (tr("Estable") if rr > -1 else tr("Decreciente"))
            rr_badge_color = COLORS["success"] if rr > 1 else (COLORS["accent"] if rr > -1 else "#a78bfa")
        else:
            rr_val = "—"
            rr_hint_text = tr("Sin datos")
            rr_label = ""
            rr_badge_color = ""
        ...
        if ftp and weight and weight > 0:
            ...
            wpkg_val = "—"
            wpkg_hint = tr("Configura tu perfil")

        sc_wpkg = StatCard(
            "⚡", tr("VATIOS POR KILO"), wpkg_val, "W/kg",
            hint=wpkg_hint,
            accent="#22d3ee",
        )
        ...
        sc_count = StatCard(
            "🚴", tr("ENTRENAMIENTOS"), str(count), "",
            hint=tr("En el periodo seleccionado"),
            accent="#a78bfa",
        )
        self._kpi_grid2.addWidget(sc_count)

        sc_time = StatCard(
            "⏱️", tr("TIEMPO TOTAL"), f"{h}h {m:02d}m", "",
        ...
        sc_dist = StatCard(
            "🛣️", tr("DISTANCIA"), f"{total_dist:.1f}", "km",
            hint=f"{avg_dist:.1f} km/{tr('sesión')}",
            accent="#22d3ee",
        )
        self._kpi_grid2.addWidget(sc_dist)

        sc_tss = StatCard(
            "🔥", tr("TSS ACUMULADO"), str(round(total_tss)), "",
            hint=f"{round(total_kj)} kJ {tr('trabajo')}",
            accent="#e879a0",
        )
    ...
    def _build_carga_forma_tab(self) -> None:
        scroll, inner, lay = _scrollable_widget()
        self._tabs.addTab(scroll, f"🔥 {tr('Carga y Forma')}")

        # CTL/ATL/TSB chart
        lay.addWidget(_section_title(
            "📈", tr("Carga de entrenamiento (CTL · ATL · TSB)"),
            tr("CTL = forma a largo plazo (42 d) · ATL = fatiga (7 d) · TSB = forma del día (CTL – ATL).")
        ))
        ...
        lay.addWidget(_section_title(
            "🔄", tr("Forma (TSB)"),
            tr("Estado del día según el balance carga–fatiga (CTL – ATL). Cada banda indica un estado de forma.")
        ))
        ...
        # ACWR chart
        lay.addWidget(_section_title(
            "🛡️", tr("Ratio de carga aguda:crónica (ACWR)"),
            tr("Relación ATL/CTL — mide cuánta carga reciente acumulas respecto a tu base crónica.")
        ))
        ...
        lay.addWidget(_section_title(
            "📊", tr("TSS semanal"),
            tr("Suma de TSS por semana ISO. El ramp rate es el ΔCTL semanal.")
        ))
    ...
    def _update_carga_forma(self) -> None:
        ...
        # Líneas
        pw.plot(xs, ctl_y, pen=pg.mkPen(CHART_COLORS["ctl"], width=2), name=tr("CTL"))
        pw.plot(xs, atl_y, pen=pg.mkPen(CHART_COLORS["atl"], width=2), name=tr("ATL"))
        pw.plot(xs, tsb_y, pen=pg.mkPen(CHART_COLORS["tsb"], width=2), name=tr("TSB"))
        ...
        def _fmt_fitness(x, _y):
            ...
            if c is not None: lines.append(tooltip_line(tr("CTL"), f"{c:.1f}", CHART_COLORS["ctl"]))
            if a is not None: lines.append(tooltip_line(tr("ATL"), f"{a:.1f}", CHART_COLORS["atl"]))
            if t is not None: lines.append(tooltip_line(tr("TSB"), f"{t:.1f}", CHART_COLORS["tsb"]))
            if ts is not None: lines.append(tooltip_line(tr("TSS"), f"{ts:.0f}", CHART_COLORS["tss"]))
        ...
        bands_labels = [
            (-30, tr("Alto riesgo"), "#ef4444"),
            (-10, tr("Óptimo"), "#22c55e"),
            (5, tr("Fresco"), "#60B5FF"),
            (20, tr("Transición"), "#a78bfa"),
        ]
        ...
        def _fmt_form(x, _y):
            ...
            lines = [
                tooltip_header(dl),
                tooltip_line(tr("TSB"), f"{v:.1f}", "#fbbf24"),
                tooltip_line(tr("Estado"), st, st_color),
            ]
            return tooltip_html(lines)
        ...
        # ACWR — solo fiable cuando CTL >= 20 (con CTL < 20 el ratio
        # es muy inestable y produce picos irreales al empezar a entrenar)
        CTL_MIN_ACWR = 20
        valid_xs = []
        valid_ys = []
        ...
        # Bandas ACWR — capear eje Y a 3.0 para que la zona óptima (0.8–1.3)
        # siempre sea visible y los picos iniciales no compriman el gráfico.
        ACWR_Y_CAP = 3.0
        raw_max = max(2.0, max(valid_ys) + 0.1) if valid_ys else 2.0
        acwr_max = min(raw_max, ACWR_Y_CAP)
        ...
        add_horizontal_band(pw3, 1.5, acwr_max, "#ef4444", 35)  # Peligro
        ...
        for y, text, col in [(0.4, tr("Infracarga"), "#60B5FF"), (1.0, tr("Óptimo"), "#22c55e"),
                              (1.35, tr("Alto"), "#FF9149"), (1.6, "Peligro", "#ef4444")]:
        ...
        def _fmt_acwr(x, _y):
            ...
            lines = [
                ...
                tooltip_line(tr("Estado"), st, st_col),
            ]
            return tooltip_html(lines)
        ...
        if weekly_tss:
            ...
            def _fmt_tss_wk(x, _y):
                ...
                lines = [
                    tooltip_header(f"{tr('Semana')} {_wk_names[idx]}"),
                    tooltip_line(tr("TSS"), f"{_wk_h[idx]:.0f}", CHART_COLORS["tss"]),
                ]
                ...
                if ramp_val is not None:
                    ...
                    lines.append(tooltip_line(tr("Ramp"), f"{ramp_sign}{ramp_val} pts/sem", ramp_color))
                return tooltip_html(lines)
    ...
    def _build_zonas_tab(self) -> None:
        scroll, inner, lay = _scrollable_widget()
        self._tabs.addTab(scroll, f"📊 {tr('Zonas')}")
        ...
        # Rangos visuales
        lay.addWidget(_section_title(
            "🔄", tr("Rangos visuales de zonas Coggan"),
            tr("Cada barra muestra desde el mínimo hasta el máximo de la zona en vatios reales.")
        ))
        self._pw_zone_ranges = make_plot(height=260)
        lay.addWidget(self._pw_zone_ranges)

        # Zone tables
        lay.addWidget(_section_title("🔄", tr("Zonas de potencia (Coggan)")))
        self._table_zones_power = self._make_zone_table()
        lay.addWidget(self._table_zones_power)

        lay.addWidget(_section_title("💜", tr("Zonas de Friel")))
    ...
    def _make_zone_table(self) -> QTableWidget:
        ...
        table.setHorizontalHeaderLabels([tr("ZONA"), "% REF", tr("RANGO")])
    ...
    def _update_zonas(self) -> None:
        ...
        # Pmax del perfil (último test de potencia), NO el máximo de actividades
        test_pmax = 0
        if self._tests and self._tests[0].max_power:
            test_pmax = self._tests[0].max_power
        ...
        hr_zone_label = "FCL" if (hr_lthr and hr_lthr > 0) else tr("FCmáx")
        self._fill_zone_table(self._table_zones_hr, HR_ZONES, hr_zone_ref, hr_unit(), hr_zone_label, 0)
    ...
    def _draw_zone_bars(
        ...
        if tooltip_key:
            ...
            def _fmt_zone(xv, _y):
                ...
                z = _zones[idx]
                lines = [tooltip_header(tr(z.label))]
                if _show_t:
                    lines.append(tooltip_line(tr("Tiempo"), _fmt_hms(_heights[idx]), z.color))
                    if _total > 0:
                        pct = _heights[idx] / _total * 100
                        lines.append(tooltip_line(tr("% total"), f"{pct:.1f}%", z.color))
                else:
                    lines.append(tooltip_line(tr("% total"), f"{_heights[idx]:.1f}%", z.color))
                    lines.append(tooltip_line(tr("Tiempo"), _fmt_hms(data.get(z.key, 0)), z.color))
                return tooltip_html(lines)
    ...
    def _draw_zone_ranges(self, ref_watts: int, max_power: int) -> None:
        ...
        n = len(POWER_ZONES)
        # Cap visual: Pmax del test si existe, si no 200% del ref (Z7 = 150–200%)
        cap = max(max_power, ref_watts * 2) if max_power > 0 else ref_watts * 2
        ...
        for i, z in enumerate(POWER_ZONES):
            ...
            hi_pct = z.max_pct if math.isfinite(z.max_pct) else (max_power / ref_watts * 100 if max_power > 0 else 200)
            hi = hi_pct / 100 * ref_watts
            hi = min(hi, cap)
            y = n - 1 - i

            self._zone_range_data.append({
                "y": y, "lo": lo, "hi": hi, "label": tr(z.label),
        ...
        def _fmt_zone_range(x, y):
            ...
            lines = [
                ...
                tooltip_line(tr("Rango"), f'{round(hit["lo"])} – {round(hit["hi"])} W', hit["color"]),
                tooltip_line(tr("% ref"), f'{hit["min_pct"]:.0f}–{hit["hi_pct"]:.0f}%', "#94a3b8"),
            ]
            return tooltip_html(lines)
    ...
    _POWER_SUBZONES = [
        {"key": "pmr", "label": tr("PMR · Potencia Mínima de Rodaje"), "min_pct": 65, "max_pct": 70, "color": "#A7C7E7", "after": "z2"},
    ...
    def _fill_zone_table(
        ...
        for z in zones:
            ...
            rows.append({
                "label": tr(z.label), "min_pct": z.min_pct, "max_pct": z.max_pct,
        ...
        for i, row in enumerate(rows):
            ...
            if math.isfinite(row["max_pct"]):
                ...
                hi = max_power if max_power > 0 else round(2.0 * ref) if ref > 0 else 0
    ...
    def _build_rendimiento_tab(self) -> None:
        scroll, inner, lay = _scrollable_widget()
        self._tabs.addTab(scroll, tr("🔬 Rendimiento"))

        # ── Modelo Critical Power card ──
        lay.addWidget(_section_title(
            "🔬", tr("Modelo Critical Power"),
            tr("Resultados del último conjunto de tests (regresión lineal: P = CP + W'/t).")
        ))
        ...
        # TTE Calculator
        lay.addWidget(_section_title(
            "⏳", tr("Calculadora TTE (Time-To-Exhaustion)"),
            tr("Estima cuánto puedes sostener una potencia dada según tu modelo CP.")
        ))
        ...
        # Records personales
        lay.addWidget(_section_title(
            "🏆", tr("Records personales"),
            tr("Tus mejores potencias sostenidas por duración. La banderita verde marca records de los últimos 30 días.")
        ))
        self._table_records = QTableWidget()
        self._table_records.setColumnCount(4)
        self._table_records.setHorizontalHeaderLabels([tr("DURACIÓN"), tr("POTENCIA"), tr("FECHA"), tr("ACTIVIDAD")])
        ...
        # Evolución CP y W'
        lay.addWidget(_section_title(
            "📈", tr("Evolución de CP y W'"),
            tr("Necesitas al menos 2 tests para ver la tendencia.")
        ))
        ...
        lay.addWidget(_section_title(
            "📈", tr("Evolución del perfil"),
            tr("Seguimiento histórico de peso, potencia y eficiencia.")
        ))
    ...
    def _build_tte_widget(self) -> QFrame:
        ...
        hint = QLabel(tr("Fórmula: TTE = W' / (P – CP)"))
        ...
        tte_lbl = QLabel(tr("TTE ESTIMADO"))
        ...
        right = QVBoxLayout()
        right.setSpacing(4)
        ref_lbl = QLabel(tr("REFERENCIAS RÁPIDAS"))
    ...
    def _update_tte_result(self) -> None:
        if not self._cp_model:
            self._tte_result_label.setText(tr("Sin modelo CP"))
            return
    ...
    def _update_cp_model_card(self) -> None:
        ...
        if not test or not model:
            placeholder = QLabel(tr("Registra tests de potencia en Ajustes para ver tu modelo CP."))
        ...
        metrics = [
            ("⚡", tr("CP"), f"{round(model.cp)} W" if model.cp else "—", "Critical Power"),
            ("🔋", tr("W'"), f"{model.w_prime_kj:.1f} kJ" if model.w_prime_kj else "—", tr("Trabajo anaeróbico")),
            ("🏔️", "P5min", f"{round(test.p5min)} W" if test.p5min else "—", "Potencia 5 min interpolada"),
            ("📊", "mFTP", f"{round(mftp)} W" if mftp else "—", tr("FTP modelado (0.96 · CP)")),
            ("🫁", "VO₂max", f"{vo2max_val:.2f}" if vo2max_val else "—", "ml/kg/min"),
            ("📐", "mFTP/VO₂max", f"{mftp_pct:.2f}%" if mftp_pct else "—", tr("Aprovechamiento aeróbico")),
            ("💥", tr("P máx"), f"{round(test.max_power)} W" if test.max_power else "—", "Pico de sprint"),
        ]
    ...
    def _update_cp_trend(self) -> None:
        ...
        if len(tests) < 2:
            txt = pg.TextItem(
                tr("Registra más tests de potencia para ver la tendencia."),
        ...
        def _fmt_cp_trend(x, _y):
            ...
            lines = [
                tooltip_header(dl),
                tooltip_line(tr("CP"), f"{_cp_v[idx]:.0f} W", CHART_COLORS["cp"]),
                tooltip_line(tr("W'"), f"{_wp_v[idx]:.1f} kJ", CHART_COLORS["w_prime"]),
            ]
            return tooltip_html(lines)
    ...
    def _update_profile_evo(self) -> None:
        ...
        def _fmt_profile(x, _y):
            ...
            lines = [
                tooltip_header(dl),
                tooltip_line(tr("FTP"), f"{_pe_ftp[idx]:.0f} W", CHART_COLORS["ftp"]),
                tooltip_line(tr("Peso"), f"{_pe_w[idx]:.1f} kg", CHART_COLORS["weight"]),
            ]
            return tooltip_html(lines)
    ...
    def _build_analisis_tab(self) -> None:
        scroll, inner, lay = _scrollable_widget()
        self._tabs.addTab(scroll, tr("🔬 Análisis avanzado"))

        # Tendencias de eficiencia
        lay.addWidget(_section_title(
            "📈", tr("Tendencias de eficiencia"),
            tr("EF (NP/FC), VF (NP/Pmedia), Pw:Hr (desacople potencia/FC) por actividad.")
        ))
        ...
        lay.addWidget(_section_title(
            "🔋", tr("PDC — Power Duration Curve"),
            tr("Potencia máxima por duración (5 s – 3 h). Basado en tus mejores esfuerzos registrados.")
        ))
        self._pw_pdc = make_plot(height=300)
        lay.addWidget(self._pw_pdc)

        # ── Selección de rangos para series ──
        lay.addWidget(_section_title(
            "🎯", tr("Selección de rangos para series"),
            tr("Banda de potencia objetivo (rosa) sobre tu curva MMP para planificar intervalos.")
        ))
    ...
    def _update_efficiency(self) -> None:
        ...
        pw.getAxis("left").setLabel(tr("EF"), color=CHART_COLORS["ef"], units=None)
        ...
        if vf_data:
            ...
            pi.getAxis("right").setLabel(tr("VF"), color=CHART_COLORS["vf"], units=None)
        ...
        def _fmt_eff(x, _y):
            ...
            if _ef:
                ...
                lines.append(tooltip_line(tr("EF"), f"{ef_ys[idx]:.2f}", CHART_COLORS["ef"]))
            if _vf:
                vf_xs = np.array([d[0] for d in _vf])
                vf_ys = np.array([d[1] for d in _vf])
                idx_v = int(np.argmin(np.abs(vf_xs - x)))
                lines.append(tooltip_line(tr("VF"), f"{vf_ys[idx_v]:.2f}", CHART_COLORS["vf"]))
            if _phr:
                ...
                lines.append(tooltip_line(tr("Pw:Hr"), f"{sign}{phr_val:.1f}%", CHART_COLORS["pwhr"]))
            return tooltip_html(lines)
    ...
    def _update_pdc(self) -> None:
        """Gráfico PDC independiente: curva de potencia-duración."""
        pw = self._pw_pdc
        pw.clear()
        ...
        # Curva PDC
        pw.plot(xs_log, ys_mmp, pen=pg.mkPen(PDC_COLOR, width=2.5), name=tr("PDC"))
        ...
        visible_ticks = [d for d in _PDC_X_TICKS if d <= max_dur]
        x_ticks = [(np.log10(d), _PDC_X_LABELS.get(d, _fmt_duration(d)))
        ...
        def _fmt_pdc(x, _y):
            ...
            lines = [
                ...
                tooltip_line(tr("PDC"), f"{pw_val} W", PDC_COLOR),
            ]
            return tooltip_html(lines)
        self._setup_tooltip("pdc", pw, _fmt_pdc)
    ...
    def _update_interval_targeting(self) -> None:
        ...
        pw.plot(xs_log, ys_low, pen=pg.mkPen("#f9a8d4", width=1,
                style=Qt.PenStyle.DashLine), name=tr("Int inf"))
        pw.plot(xs_log, ys_high, pen=pg.mkPen("#f472b6", width=1,
                style=Qt.PenStyle.DashLine), name=tr("Int sup"))

        # Curva MMP (referencia para las bandas de targeting)
        pw.plot(xs_log, ys_mmp, pen=pg.mkPen("#ef4444", width=2.5), name=tr("MMP"))
        ...
        def _fmt_it(x, _y):
            ...
            lines = [
                tooltip_header(t_str),
                tooltip_line(tr("MMP"), f"{pw_val} W", "#ef4444"),
                tooltip_line(tr("Int sup"), f"{round(pw_val * hi_pct)} W", "#f472b6"),
                tooltip_line(tr("Int inf"), f"{round(pw_val * lo_pct)} W", "#f9a8d4"),
            ]
            return tooltip_html(lines)
    ...
    def _build_entrenamientos_tab(self) -> None:
        scroll, inner, lay = _scrollable_widget()
        self._tabs.addTab(scroll, f"🚴 {tr('Entrenamientos')}")
        ...
        lay.addWidget(_section_title(
            "📅", tr("Calendario de entrenamiento"),
        ))
        ...
        lay.addWidget(_section_title(
            "📋", tr("Últimos entrenamientos"),
            tr("Tus últimas sesiones procesadas.")
        ))
        self._table_recent = QTableWidget()
        self._table_recent.setColumnCount(5)
        self._table_recent.setHorizontalHeaderLabels(
            [tr("ACTIVIDAD"), tr("FECHA"), tr("DURACIÓN · DISTANCIA"), tr("NP · IF"), tr("TSS")]
        )
        ...
        self._table_recent.doubleClicked.connect(self._on_activity_double_click)
        self._table_recent.clicked.connect(self._on_activity_double_click)
    ...
    def _update_calendar(self) -> None:
        ...
        month_names = ["", tr("Enero"), tr("Febrero"), tr("Marzo"), tr("Abril"), tr("Mayo"), tr("Junio"),
        ...
        for d_name in [tr("LUN"), tr("MAR"), tr("MIÉ"), tr("JUE"), tr("VIE"), tr("SÁB"), tr("DOM")]:
            lbl = QLabel(d_name)
    ...
    def _update_recent_activities(self) -> None:
        ...
        for i, a in enumerate(recent):
            # Activity name (left-aligned) — store ID in UserRole for click handler
    ...
    def _on_activity_double_click(self, index) -> None:
        """Abre detalle de la actividad al hacer clic."""
        row = index.row()
        print(f"[Dashboard] click on row={row}")
        try:
            name_item = self._table_recent.item(row, 0)
            if name_item is not None:
                act_id = name_item.data(Qt.ItemDataRole.UserRole)
                if act_id is not None:
                    print(f"[Dashboard] Opening activity id={act_id}")
                    self.open_activity.emit(int(act_id))
                    return
            # Fallback
            recent = self._activities_display[:20]
            if 0 <= row < len(recent):
                print(f"[Dashboard] Fallback: opening id={recent[row].id}")
                self.open_activity.emit(int(recent[row].id))

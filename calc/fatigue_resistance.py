...
def calc_fatigue_resistance(power: List[float]) -> FatigueResistanceResult:
    ...
    if len(clean) < MIN_SAMPLES * 2:
        return FatigueResistanceResult(
            ...
            classification="insufficient", class_label=tr("Sin datos"),
        )
...
def classify_fr(fr: Optional[float]) -> Tuple[str, str]:
    if fr is None:
        return ("insufficient", tr("Sin datos"))
    if fr >= 0.95:
        return ("excellent", tr("Excelente"))
    if fr >= 0.90:
        return ("good", tr("Buena"))
    if fr >= 0.85:
        return ("normal", tr("Normal"))
    if fr >= 0.80:
        return ("moderate_fade", tr("Fade moderado"))
    return ("significant_fade", tr("Fade significativo"))
